...
const callWebFrameMethod = function (requestId, method, callback, ...args) {
  return new Promise((resolve, reject) => {
    ipcMainInternal.once(`ELECTRON_INTERNAL_BROWSER_WEB_FRAME_RESPONSE_${requestId}`, function (event, error, result) {
      if (error == null) {
    ...
    this._sendInternal('ELECTRON_INTERNAL_RENDERER_WEB_FRAME_METHOD', requestId, method, callback !== false, args)
  })
}

for (const method of syncWebFrameMethods) {
  WebContents.prototype[method] = function (...args) {
    return callWebFrameMethod.call(this, getNextId(), method, false, ...args)
  }
}
...
WebContents.prototype.executeJavaScript = function (code, hasUserGesture, callback) {
  ...
  if (this.getURL() && !this.isLoadingMainFrame()) {
    return callWebFrameMethod.call(this, requestId, 'executeJavaScript', callback, code, hasUserGesture)
  } else {
    return new Promise((resolve, reject) => {
      this.once('did-stop-loading', () => {
        callWebFrameMethod.call(this, requestId, 'executeJavaScript', callback, code, hasUserGesture).then(resolve).catch(reject)
      })
    })
  }
}
