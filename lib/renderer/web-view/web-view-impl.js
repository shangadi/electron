...
const setupMethods = (WebViewElement) => {
  ...
  const createBlockHandler = function (method) {
    return function (...args) {
      return ipcRendererUtils.invokeSync('ELECTRON_GUEST_VIEW_MANAGER_SYNC_CALL', getGuestInstanceId(this), method, args)
    }
  }
  ...
  const createNonBlockHandler = function (method) {
    return function (...args) {
      ...
      ipcRendererUtils.invoke('ELECTRON_GUEST_VIEW_MANAGER_ASYNC_CALL', getGuestInstanceId(this), method, args, callback != null)
        .then(result => {
